...
class ScaledL2Trans(Module):
    def __init__(self, original_dimension: int = 256, embedding_dimension: int = 16, to_scale: bool = False) -> None:
    ...
    def forward(self, database, query, db_embedding, query_embedding):
        original_l2 = self.__l2(squeeze(database), squeeze(
            query)) / self.__scale_factor_original
        embedding_l2 = self.__l2(squeeze(db_embedding), squeeze(
            query_embedding)) / self.__scale_factor_embedding

        return self.__l1(original_l2.view([1, -1]), embedding_l2.view([1, -1]))[0] / database.shape[0]


class ScaledL2Recons(Module):
    def __init__(self, original_dimension: int = 256, to_scale: bool = False) -> None:
